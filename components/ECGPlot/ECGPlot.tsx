...
const drawHighlights = (
   ...
   const highlightLayer = g.append('g').attr('class', 'highlight-layer');

   events.forEach((evt) => {
      // ensure evt.start <= evt.end
...
/** Helper to create a Y-scale for each lead, with amplitude padding so wave isn't clipped. */
...
const EcgChart = ({ data, events }: Props) => {
   ...
   useEffect(() => {
      ...
      drawGrid(g, innerWidth, innerHeight); // background grid
      drawHighlights(g, events, xScale, innerHeight); // translucent yellow boxes
      drawLeads(g, data, xScale, bandHeight, margin.left); // ECG traces & labels
      drawXAxis(g, xScale, innerHeight); // bottom axis
   }, [data, events]);
